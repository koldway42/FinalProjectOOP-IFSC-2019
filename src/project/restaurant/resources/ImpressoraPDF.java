package project.restaurant.resources;
...
public class ImpressoraPDF {
	...
	private static <E> void criarTabela(Document documento, String[] cabecalho, List<E> dados)
		...
		tabela.setWidths(new int[]{10,30,30,60});
		for (E e : dados) {
			if (e instanceof User) {
				tabela.addCell(((User) e).getId().toString());
				tabela.addCell(((User) e).getName());
				tabela.addCell(((User) e).getFormattedCreatedAt());
				tabela.addCell(((User) e).getEmail());
			}
		}
